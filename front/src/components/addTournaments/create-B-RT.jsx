...
const BattleRoyalT=({setType})=>{
    ...
    const Submit = async(e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3000/auth/user/create/tournament/battle-royal/team', {
                name,
                team_name,
                game,
                num_of_part,
...
return(
        ...
        <h1>Форма для Турнира Battle Royal командный</h1>
        <hr></hr>
...
<label for="num_of_part"><b>Максимальное кол-во команд</b></label>
        <input type="text" id="num_of_part" 
        placeholder="Введите кол-во людей в команде"
        value={num_of_part} 
        onChange={(e)=>setNum_of_part(e.target.value)}
        name="num_of_part" required/>
