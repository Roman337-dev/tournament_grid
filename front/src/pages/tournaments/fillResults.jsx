...
const FillResults = () => {
    ...
    const fetchParticipants = async (tourn_id, type) => {
			try {
					const response = await axios.get(`http://localhost:3000/auth/fill/results?id=${tourn_id}&type=${type}` , {
                        withCredentials: true
                    });
					const participantsData = response.data.participants.map(email => ({ email, score: '' }));
    ...
    const handleSubmit = async (e, index) => {
        ...
        try {
            await axios.post('http://localhost:3000/auth/user/rounds/tournament/addresult', 
            {
...
return (<>
	<header>
            ...
            <aside id="profile-section">
				...
				{isRet?(<><Link className="link" to={`/auth/fill/results?id=${tourn_id}&type=${type}`}> <h3>Заполнить результаты</h3></Link>
				<Link className="link" to={`/auth/get/results?id=${tourn_id}&type=${type}`}> <h3>Посмотреть результаты</h3></Link></>):(<></>)}

				{participant.map((participant, index) => (
                    <div id="participant-container" key={index}>
