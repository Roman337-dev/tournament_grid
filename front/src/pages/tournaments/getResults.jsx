...
const GetResult = ()=>{
    ...
    useEffect(() => {
        ...
        setType(type);
    fetchRes(tourn_id, type);
    
    }, [location.search]);

    const fetchRes = async (tourn_id, type) => {
        try {
            const response = await axios.get(`http://localhost:3000/auth/get/results?id=${tourn_id}&type=${type}`, {
        ...
        <>
         <header>
            ...
            <h2 id="h2">Результаты турнира</h2>
                ...
                {isAdmin?(<><Link className="link" to={`/auth/fill/results?id=${tourn_id}&type=${type}`}> <h3>Редактировать результаты</h3></Link></>):(<></>)}

                <h2 id="h1">Топ 3 игрока</h2>
                <div id="participant-container">
                   <ol> {top3.map((top)=>(
                        <li>
                             <p class="id">{top.participantemail} </p>
                        </li>
                    ))}
                    </ol>
                </div>
                    ...
                    {results.map((res)=>(
                        <li>
                            <div id="participant-container">
                            <p class="id">{res.participantemail} </p>
